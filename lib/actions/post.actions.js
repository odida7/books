...
export const deletePost = async(id) => {
  try{
    ...
    return { error: `Error deleting post: ${err.message}`, status: 500 };
  }
}
...
export const updatePost = async(id, formData) => {
  ...
  try{
     await connectDB();
     const update = await Post.findByIdAndUpdate(id, {userId, text, createdAt}, {new: true});
     return{
       update,
    ...
    return { error: `Error updating post: ${err.message}`, status: 500 };
  }
}